...
export function encodingLabel(head: string, sniffed: SniffedValue | null, chars: number): string {
  const size = i18n.t("largeValue.charCount", { count: chars.toLocaleString() });
  if (sniffed?.encoding === "percent") {
    return i18n.t("largeValue.percentEncoded", { size });
  }
  const encoded = sniffed?.encoding === "base64" || isEncodedRun(head, sniffed?.offset ?? 0);
  return encoded ? i18n.t("largeValue.base64Size", { size }) : size;
}
...
export function largeValueActions({
  ...
  if (onView != null) {
    items.push({
      label:
        sniffed == null
          ? i18n.t("largeValue.view")
          : i18n.t("largeValue.viewType", { label: sniffed.label }),
  ...
  if (image) {
    items.push({
      label: i18n.t("largeValue.copyImage"),
  ...
  items.push({
    label:
      sniffed?.encoding === "base64" ? i18n.t("largeValue.copyBase64") : i18n.t("response.copy"),
  ...
  items.push({
    label: i18n.t("response.saveToFile"),
  ...
  navigator.clipboard
    ...
    .then(() =>
      showToast({
        ...
        message: i18n.t("largeValue.copiedAsPng"),
      }),
    )
    .catch((err: unknown) => {
...
export async function saveValue(text: string, sniffed: SniffedValue | null, name: string) {
  ...
  const filepath = await save({
    ...
    title: i18n.t("largeValue.saveValue"),
  });
  if (filepath == null) {
    return; // Cancelled
  }
  ...
  showToast({ message: i18n.t("largeValue.savedTo", { path: filepath }) });
}
